...
namespace NiantiSpatial.NSDK.Editor
{
#if UNITY_ANDROID && UNITY_EDITOR
  // Modify the build time AndroidManifest to include Nsdk required queries and dependencies
  ...
  internal class NsdkPostGenerateAndroidManifest:
    IPostGenerateGradleAndroidProject
  {
    ...
    // Append any missing Nsdk requirements to the merged AndroidManifest
...
// version is adequate for what we need and we can use it versus our custom version
    ...
    // Contains Nsdk specific functionality
      ...
      AndroidXmlDocument
    {
      ...
      internal bool AddQuery(string packageName)
      {
        ...
        if (packageName == NsdkPostGenerateAndroidManifest.ARCoreNameString)
        {
          Log.Info("NsdkPostGenerateAndroidManifest.AddQuery skipped adding ARCore on OpenXR platforms");
          return changed;
        }
#endif
      ...
      // Nsdk specific helper to add ARCore install activity and ARCore metadata as optional
      internal bool AddARCoreInstallActivity()
      {
...
#if MAGICLEAP || METAQUEST
            Log.Info("NsdkPostGenerateAndroidManifest.AddARCoreInstallActivity skipped on OpenXR platforms");
            return changed;
#endif

        var installActivityBlock = SelectNodes
        (
          $"/manifest/application/activity[@android:name='{NsdkPostGenerateAndroidManifest.ARCoreInstallActivityString}']",
          nsMgr
        );
        if (installActivityBlock?.Count == 0)
        {
          ...
          (
            CreateAndroidAttribute
              ("name", NsdkPostGenerateAndroidManifest.ARCoreInstallActivityString)
          );

          elem.Attributes.Append
          (
            CreateAndroidAttribute
              ("configChanges", NsdkPostGenerateAndroidManifest.ARCoreInstallActivityConfigString)
          );
          ...
          (
            CreateAndroidAttribute
              ("launchMode", NsdkPostGenerateAndroidManifest.ARCoreInstallActivityLaunchModeString)
          );
        ...
        (
          $"/manifest/application/meta-data[@android:name='{NsdkPostGenerateAndroidManifest.ARCoreNameString}']",
          nsMgr
        );
        if (arcoreMetadata?.Count == 0)
        {
          ...
          metadataElem.Attributes.Append
            (CreateAndroidAttribute("name", NsdkPostGenerateAndroidManifest.ARCoreNameString));
      ...
      internal bool AddOpenClNativeLibraries()
      {
        ...
        (
          $"/manifest/application/uses-native-library[@android:name='{NsdkPostGenerateAndroidManifest.OpenClPixelLibraryString}']",
          nsMgr
        );
        if (applicationBlockOpenClPixel?.Count == 0)
        {
          ...
          elem.Attributes.Append
            (CreateAndroidAttribute("name",
              NsdkPostGenerateAndroidManifest.OpenClPixelLibraryString)
          );
        ...
        (
          $"/manifest/application/uses-native-library[@android:name='{NsdkPostGenerateAndroidManifest.OpenClLibraryString}']",
          nsMgr
        );
        if (applicationBlockOpenCl?.Count == 0)
        {
          ...
          elem.Attributes.Append
            (CreateAndroidAttribute ("name",
              NsdkPostGenerateAndroidManifest.OpenClLibraryString)
          );
          elem.Attributes.Append(CreateAndroidAttribute("required", "false"));
