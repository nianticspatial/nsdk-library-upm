Shader "Nsdk/SceneSegmentationOverlay"
{
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag (v2f i) : SV_Target
            {
                ...
                float2 sceneSegmentation_uv = float2(i.uv.x / i.uv.z, i.uv.y / i.uv.z);

                // Calculate a mask such that, if UV is outside [0,1] region, return zero confidence
                float2 inside = step(0.0, sceneSegmentation_uv) * step(sceneSegmentation_uv, 1.0);
                float clampMask = inside.x * inside.y;

                // Sample the semantics texture
                float confidence = tex2D(_SceneSegmentation, sceneSegmentation_uv) * clampMask;
