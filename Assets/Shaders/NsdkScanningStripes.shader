Shader "Unlit/NsdkScanningStripes"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            inline float2 GetSampleUV(float2 uv, float2 inSize, float2 outSize)
            {
            ...
            fixed4 frag (v2f i, UNITY_VPOS_TYPE screenPos : VPOS) : SV_Target
            {
                ...
                float2 cameraUv = GetSampleUV(i.uv, _ArCameraTex_TexelSize.zw, outSize);
                float2 raycastUv = GetSampleUV(i.uv, _ColorTex_TexelSize.zw, outSize);
                const half4 cameraColor = tex2D(_ArCameraTex, cameraUv);
                const half conf = tex2D(_ColorTex, raycastUv).w;
