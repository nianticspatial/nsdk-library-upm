...
namespace NianticSpatial.NSDK.AR.Subsystems.WorldPositioning
{
    public class NativeApi : IApi
    {
        ...
        {
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_Create")]
            public static extern IntPtr Create(IntPtr unity_context);

            // Release WPS Feature
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_Release")]
            public static extern void Release(IntPtr feature_handle);

            // Start WPS data processing
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_Start")]
            public static extern void Start(IntPtr feature_handle);

            // Stop WPS data processing
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_Stop")]
            public static extern void Stop(IntPtr feature_handle);

            // Configure WPS
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_Configure")]
            public static extern void Configure(IntPtr handle, WpsConfigurationCStruct config);

            [DllImport
                (NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_WPS_GetLatestTransform")]
