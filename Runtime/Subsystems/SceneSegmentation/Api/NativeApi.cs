...
namespace NianticSpatial.NSDK.AR.Subsystems.SceneSegmentation
{
    ...
    internal class NativeApi : IApi
    {
        ...
        private static readonly List<SceneSegmentationChannel> AllChannels = new List<SceneSegmentationChannel>
        {
            SceneSegmentationChannel.Sky,
            SceneSegmentationChannel.Ground,
            SceneSegmentationChannel.NaturalGround,
            SceneSegmentationChannel.ArtificialGround,
            SceneSegmentationChannel.Grass,
        };

        private readonly Dictionary<SceneSegmentationChannel, int> _sceneSegmentationChannelToShaderPropertyIdMap = new ();
        private readonly Dictionary<SceneSegmentationChannel, BufferedTextureCache> _sceneSegmentationBufferedTextureCaches = new ();

        private readonly int _packedSceneSegmentationPropertyNameID = Shader.PropertyToID(TexturePackedSemanticChannelPropertyName);
        private readonly int _suppressionMaskPropertyNameID = Shader.PropertyToID("_SuppressionMask");
        private BufferedTextureCache _packedSceneSegmentationBufferedTextureCache = new BufferedTextureCache(FramesInMemory);
        ...
        /// number of channels in the <c>SceneSegmentationChannel</c> enum. </param>
        /// <param name="thresholds"> An array of float values. Each index in the array corresponds to a channel in the
        /// <c>SceneSegmentationChannel</c> enum. A negative value will have no effect and will
        ...
        /// </param>
        /// <param name="sceneSegmentationChannelDescriptor"> The semantic channel texture descriptor to be populated, if available. </param>
        ...
        {
            sceneSegmentationChannelDescriptor = default;
            samplerMatrix = Matrix4x4.identity;
            var resourceHandle =
                Native.GetSceneSegmentationChannel
                (
            ...
            var texture =
                _sceneSegmentationBufferedTextureCaches[channel].GetUpdatedTextureFromBuffer
                (
                    memoryBuffer,
                    size,
                    width,
            ...
            // Package results
            sceneSegmentationChannelDescriptor = new XRTextureDescriptor
            (
                ...
                0,
                format,
                propertyNameId: _sceneSegmentationChannelToShaderPropertyIdMap[channel],
                depth: 0,
        ...
        /// image will not be warped.
        /// </param>
        /// <param name="packedSceneSegmentationDescriptor"> The packed semantics texture descriptor to be populated, if available. </param>
        /// <param name="samplerMatrix">
        /// A matrix that converts from normalized viewport coordinates to normalized texture coordinates.
        /// </param>
        /// <returns>
        ...
        {
            packedSceneSegmentationDescriptor = default;
            samplerMatrix = Matrix4x4.identity;

            var resourceHandle =
                Native.GetPackedSceneSegmentationChannels
                (
                    nativeProviderHandle,
                    out var memoryBuffer,
                    out var size,
            ...
            var texture = _packedSceneSegmentationBufferedTextureCache.GetUpdatedTextureFromBuffer
            (
                memoryBuffer,
                size,
                width,
            ...
            packedSceneSegmentationDescriptor = new XRTextureDescriptor
            (
                texture.GetNativeTexturePtr(),
                width,
                height,
                0,
                format,
                _packedSceneSegmentationPropertyNameID,
                0,
        ...
        public List<SceneSegmentationChannel> GetChannels()
        {
            ...
            return new List<SceneSegmentationChannel>(AllChannels);
        }
        ...
        public uint GetFlags(IEnumerable<SceneSegmentationChannel> channels)
        {
            if (channels == null)
            {
                return 0u;
        ...
        private void SetMetadataDependencies()
        {
            ...
            foreach (SceneSegmentationChannel channel in Enum.GetValues(typeof(SceneSegmentationChannel)))
            {
                ...
                _sceneSegmentationChannelToShaderPropertyIdMap[channel] =
                    Shader.PropertyToID(TextureSemanticChannelPropertyPrefix + channelName);

                _sceneSegmentationBufferedTextureCaches[channel] =
                    new BufferedTextureCache(FramesInMemory);
            }
        }

        /// <summary>
        /// Converts a SceneSegmentationChannel enum value to its string representation.
        /// </summary>
        /// <param name="channel">The SceneSegmentationChannel enum value.</param>
        /// <returns>The string representation of the channel (e.g., "sky", "ground", "natural_ground").</returns>
        private static string GetChannelNameFromEnum(SceneSegmentationChannel channel)
        {
            switch (channel)
            {
        ...
        /// <summary>
        /// Converts a channel name string to a SceneSegmentationChannel enum value.
        /// </summary>
        /// <param name="channelName">The channel name string (e.g., "sky", "ground", "natural_ground").</param>
        /// <param name="channel">The corresponding SceneSegmentationChannel enum value if found.</param>
        /// <returns>True if the channel name was successfully parsed, false otherwise.</returns>
        private static bool TryParseChannelName(string channelName, out SceneSegmentationChannel channel)
        {
            ...
            {
                case "sky":
                    channel = SceneSegmentationChannel.Sky;
                    return true;
                case "ground":
                    channel = SceneSegmentationChannel.Ground;
                    return true;
                case "naturalground":
                    channel = SceneSegmentationChannel.NaturalGround;
                    return true;
                case "artificialground":
                    channel = SceneSegmentationChannel.ArtificialGround;
                    return true;
                case "grass":
                    channel = SceneSegmentationChannel.Grass;
                    return true;
                default:
                    channel = SceneSegmentationChannel.Sky; // Default fallback
                    return false;
            }
        }
        ...
        {
            _packedSceneSegmentationBufferedTextureCache?.Dispose();
            _suppressionMaskBufferedTextureCache?.Dispose();

            foreach (var bufferedTextureCache in _sceneSegmentationBufferedTextureCaches.Values)
                bufferedTextureCache.Dispose();


            _sceneSegmentationBufferedTextureCaches.Clear();
            _sceneSegmentationChannelToShaderPropertyIdMap.Clear();
        ...
        private static class Native
        {
            ...
            public static extern IntPtr GetSceneSegmentationChannel
            (
                IntPtr nativeProviderHandle,
                SceneSegmentationChannel channel,
                out IntPtr memoryBuffer,
            ...
            public static extern IntPtr GetPackedSceneSegmentationChannels
            (
                IntPtr nativeProviderHandle,
                out IntPtr memoryBuffer,
                out int size,
