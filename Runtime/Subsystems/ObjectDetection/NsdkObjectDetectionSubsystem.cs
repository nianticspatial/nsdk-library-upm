...
{
    public class NsdkObjectDetectionSubsystem : XRObjectDetectionSubsystem, ISubsystemWithMutableApi<IApi>
    {
        ...
        {
            get => ((NsdkObjectDetectionProvider)provider).FramesUntilSeenByFilter;
            set => ((NsdkObjectDetectionProvider)provider).FramesUntilSeenByFilter = value;
        }

        internal uint FramesUntilDiscardedByFilter
        {
            get => ((NsdkObjectDetectionProvider)provider).FramesUntilDiscardedByFilter;
            set => ((NsdkObjectDetectionProvider)provider).FramesUntilDiscardedByFilter = value;
        }

        /// <summary>
        /// Register the NSDK object detection subsystem.
        /// </summary>
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
        private static void Register()
        {
            Log.Info(nameof(NsdkObjectDetectionSubsystem) + "." + nameof(Register));
            const string id = "Nsdk-ObjectDetection";
            var cinfo = new XRObjectDetectionSubsystemCinfo()
            {
                id = id,
                providerType = typeof(NsdkObjectDetectionProvider),
                subsystemTypeOverride = typeof(NsdkObjectDetectionSubsystem),
        ...
        {
            ((NsdkObjectDetectionProvider)provider).SwitchApiImplementation(api);
        }
        ...
        internal class NsdkObjectDetectionProvider : Provider
        {
            ...
            {
                private readonly NsdkObjectDetectionProvider _provider;
                ...
                public DisplayHelper(NsdkObjectDetectionProvider provider)
                {
                    _provider = provider;
                }
            }
            ...
            public NsdkObjectDetectionProvider() : this(new NativeApi()) { }

            /// <summary>
            /// Construct the implementation provider.
            /// </summary>
            public NsdkObjectDetectionProvider(IApi api)
            {
                Log.Debug("Constructing NsdkObjectDetectionSubsystem.NsdkObjectDetectionProvider...");

                _api = api;
#if NIANTICSPATIAL_NSDK_AR_LOADER_ENABLED
                _nativeProviderHandle = _api.Construct(NsdkUnityContext.UnityContextHandle);
#endif
                Log.Debug("NsdkObjectDetectionProvider constructed with nativeProviderHandle: " + _nativeProviderHandle);
            ...
            {
                // Reset categories, so that anything checking and relying on these names
                // cant access them when the system isnt running
            ...
            )
            {
                ...
                for (int i = 0; i < numDetections; i++)
                {
                    ...
                    results[i] = new NsdkDetectedObject(id, rect, confidences, _categoryDirectory, _displayHelper, container);
                }
