...
namespace NianticSpatial.NSDK.AR.Subsystems.PersistentAnchor
{
    internal class NativeApi : IApi
    {
        ...
        public void TryCreateAnchor(IntPtr anchorProviderHandle, Pose pose, out TrackableId anchorId)
        {
            ...
            float[] poseArray = MatrixConversionHelper.Matrix4x4ToInternalArray(poseMatrix.FromUnityToNsdk());
        ...
        )
        {
            ...
            var matrix = MatrixConversionHelper.InternalToMatrix4x4(poseArray.ToArray()).FromNsdkToUnity();
        ...
        public bool GetVpsDebuggerLog(IntPtr anchorProviderHandle, out string vpsDebuggerLog)
        {
            ...
            if (bufHandle.IsValidHandle() && len > 0 && bufPtr.IsValidHandle())
            {
                ...
                NsdkExternUtils.ReleaseResource(bufHandle);
                return true;
            }
            vpsDebuggerLog = null;
            return false;
        ...
        {
            // Convert Unity pose to NSDK coordinate space and decompose into TransformCStruct
            var nsdkMatrix = poseMatrix.FromUnityToNsdk();
            var transformStruct = new TransformCStruct();
            transformStruct.SetTransform(nsdkMatrix);
        ...
        {
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_Construct")]
            public static extern IntPtr Construct(IntPtr unityContext);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_Start")]
            public static extern void Start(IntPtr anchorApiHandle);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_Stop")]
            public static extern void Stop(IntPtr anchorApiHandle);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_Configure")]
            public static extern void Configure
            (IntPtr anchorProviderHandle,
                AnchorConfigurationCStruct config
            );

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_Destruct")]
            public static extern void Destruct(IntPtr anchorApiHandle);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_TryCreateAnchor")]
            public static extern void TryCreateAnchor
                (IntPtr anchorApiHandle, float[] poseMatrix, out TrackableId anchorId);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_TryRemoveAnchor")]
            public static extern bool TryRemoveAnchor(IntPtr anchorApiHandle, ref TrackableId anchorId);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_TryTrackAnchor")]
            ...
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_AcquireLatestChanges")]
            ...
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_ReleaseLatestChanges")]
            public static extern void ReleaseLatestChanges(IntPtr changesHandle);

            [DllImport
                (NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_AnchorProvider_TryExtractAnchorChange")]
