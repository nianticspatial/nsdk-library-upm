...
{
    /// <summary>
    /// The NSDK implementation of the <c>XRSemanticsSubsystem</c>. Do not create this directly.
    /// Use the <c>SubsystemManager</c> instead.
    /// </summary>
    [Preserve]
    public sealed class NsdkSemanticsSubsystem : XRSemanticsSubsystem, ISubsystemWithMutableApi<IApi>
    {
        ...
        {
            Log.Info(nameof(NsdkSemanticsSubsystem) + "." + nameof(Register));
            const string id = "Nsdk-Semantics";
            var xrSemanticsSubsystemCinfo = new XRSemanticsSubsystemCinfo()
            {
                id = id,
                providerType = typeof(NsdkSemanticsProvider),
                subsystemTypeOverride = typeof(NsdkSemanticsSubsystem),
        ...
        public Matrix4x4? LatestIntrinsicsMatrix
        {
            get
            {
                if (provider is NsdkSemanticsProvider nsdkProvider)
                {
        ...
        {
            ((NsdkSemanticsProvider)provider).SwitchApiImplementation(api);
        }

        void ISubsystemWithMutableApi<IApi>.SwitchToInternalMockImplementation()
        {
            ((NsdkSemanticsProvider)provider).SwitchApiImplementation(new MockApi());
        }

        /// <summary>
        /// The implementation provider class.
        /// </summary>
        internal class NsdkSemanticsProvider : Provider
        {
            /// <summary>
            ...
            private List<SemanticsChannel> _channels = new();
            private HashSet<SemanticsChannel> _suppressionMaskChannels = new();

            public NsdkSemanticsProvider() : this(new NativeApi()) { }
            ...
            public override HashSet<SemanticsChannel> SuppressionMaskChannels
            {
                ...
                set
                {
                    ...
                    if (overwrite)
                    {
                        ...
                        _suppressionMaskChannels = value == null ? new HashSet<SemanticsChannel>() : new HashSet<SemanticsChannel>(value);
            ...
            /// </summary>
            public NsdkSemanticsProvider(IApi api)
            {
                Log.Info("NsdkSemanticsSubsystem.NsdkSemanticsProvider construct");

                _api = api;
#if NIANTICSPATIAL_NSDK_AR_LOADER_ENABLED
                _nativeProviderHandle = _api.Construct(NsdkUnityContext.UnityContextHandle);
#endif
                Log.Info("NsdkSemanticsSubsystem got nativeProviderHandle: " + _nativeProviderHandle);
            ...
            internal void SwitchApiImplementation(IApi api)
            {
                ...
                _channels = new List<SemanticsChannel>();
            }
            ...
            /// </summary>
            public override IReadOnlyList<SemanticsChannel> GetChannels()
            {
            ...
            public override bool TrySetChannelConfidenceThresholds(Dictionary<SemanticsChannel, float> channelConfidenceThresholds)
            {
                if (!_nativeProviderHandle.IsValidHandle())
                {
                    return false;
                ...
                foreach (var channelThresholdPair in channelConfidenceThresholds)
                {
                    ...
                    var semanticChannel = channelThresholdPair.Key;
                    ...
                    var index = channels.IndexOf(semanticChannel);

                    if (index < 0)
                    {
                        Log.Error
                        (
                            $"Semantic channel {semanticChannel} was not found in the list of semantic channels"
                        );
