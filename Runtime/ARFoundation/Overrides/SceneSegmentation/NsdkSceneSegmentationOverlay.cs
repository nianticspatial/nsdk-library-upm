...
{
    public sealed class NsdkSceneSegmentationOverlay : ConditionalRenderer
    {
        private static readonly int s_sceneSegmentationTextureId = Shader.PropertyToID("_SceneSegmentation");
        ...
        /// </summary>
        public const string KShaderName = "Nsdk/SceneSegmentationOverlay";
        ...
        {
            get => "Nsdk SceneSegmentation Overlay";
        }
        ...
        // Helpers
        private SceneSegmentationChannel[] _channels;
        ...
        /// Sets the current channel to display.
        /// </summary>
        public void SetChannel(SceneSegmentationChannel channel)
        {
            if (_channels != null)
            {
        ...
        protected override void Awake()
        {
            ...
            _renderPass = new MeshRenderingPass(
                name: "Nsdk SceneSegmentation Overlay",
        ...
        protected override void Update()
        {
            ...
            if (!_sceneSegmentationSubsystem.running)
            {
                return;
            }
            ...
            {
                var channels = _sceneSegmentationSubsystem.GetChannels();
                    ...
                    out var samplerMatrix))
            {
                ...
                Material.SetTexture(s_sceneSegmentationTextureId, texture);
        ...
        private bool FetchSemanticsData(SceneSegmentationChannel channel, out Texture2D texture, out Matrix4x4 intrinsicsMatrix, out Matrix4x4 samplerMatrix)
        {
            // Acquire the latest image
            if (_sceneSegmentationSubsystem.TryAcquireSceneSegmentationChannelCpuImage(
                    channel: channel,
                    ...
                    samplerMatrix: out samplerMatrix))
            {
                ...
                var intrinsics = _sceneSegmentationSubsystem.LatestIntrinsicsMatrix;
        ...
        {
            mat.SetTexture(s_sceneSegmentationTextureId, null);
        }
        ...
        private bool TryAcquireSubsystems()
        {
            ...
            if (_sceneSegmentationSubsystem == null)
            {
                Debug.LogError
                (
                    "Destroying XRSceneSegmentationOverlay component because " +
                    $"no active {typeof(XRSceneSegmentationSubsystem).FullName} is available. " +
