...
namespace NianticSpatial.NSDK.AR.ObjectDetection
{
    /// <summary>
    /// The manager for the object detection subsystem.
    /// </summary>
    [PublicAPI("apiref/Niantic/NSDK/AR/ObjectDetection/ARObjectDetectionManager/")]
    [DisallowMultipleComponent]
    [DefaultExecutionOrder(NsdkARUpdateOrder.ObjectDetectionManager)]
        ...
        SubsystemLifecycleManager<XRObjectDetectionSubsystem, XRObjectDetectionSubsystemDescriptor, XRObjectDetectionSubsystem.Provider>
    {
        ...
        [Range(1, 90)]
        private uint _targetFrameRate = NsdkObjectDetectionSubsystem.MaxRecommendedFrameRate;
        ...
        /// not officially supported, and may be deprecated without notice.
        /// </remarks>
        public bool AreCategoryNamesAvailable => subsystem?.IsMetadataAvailable ?? false;
        ...
        /// not officially supported, and may be deprecated without notice.
        /// </remarks>
        public event Action<ARObjectDetectionModelEventArgs> CategoryNamesAvailable
        {
            add
            {
                _categoryNamesAvailable += value;
                if (AreCategoryNamesAvailable)
                {
                    var args =
        ...
        private Action<ARObjectDetectionModelEventArgs> _categoryNamesAvailable;
        private bool _sentCategoryNamesAvailableEvent;
        ...
        {
            base.OnDisable();
            _sentCategoryNamesAvailableEvent = false;
        }
        ...
        private void Update()
        {
            ...
            if (!_sentCategoryNamesAvailableEvent)
            {
                var args = new ARObjectDetectionModelEventArgs { CategoryNames = CategoryNames };
                if (args.CategoryNames.Count > 0)
                {
                    _sentCategoryNamesAvailableEvent = true;
                    _categoryNamesAvailable?.Invoke(args);
                }
                else
                {
