...
namespace NianticSpatial.NSDK.AR.Occlusion
{
    ...
    /// </summary>
    [Obsolete("Use NsdkOcclusionExtension instead.")]
    [RequireComponent(typeof(AROcclusionManager))]
    public class NsdkOcclusionEffect : NsdkPostBackgroundRenderer
    {
        ...
        public const string KShaderName = "Nsdk/OcclusionEffect";
        ...
        {
            get => "NSDK Occlusion Effect";
        }

        // Error messages
        private const string k_TargetFrameRateNotSupportedMessage =
            "TargetFrameRate is not supported on non-NSDK implementations of the XROcclusionSubsystem.";
        ...
        public uint TargetFrameRate
        {
            get
            {
                if (_occlusionSubsystem is NsdkOcclusionSubsystem nsdkOcclusionSubsystem)
                {
                    return nsdkOcclusionSubsystem.TargetFrameRate;
                }
            ...
            set
            {
                ...
                if (_occlusionSubsystem is NsdkOcclusionSubsystem nsdkOcclusionSubsystem)
                {
                    nsdkOcclusionSubsystem.TargetFrameRate = value;
                }
        ...
        {
            // Using NSDK occlusion subsystem
            if (_occlusionSubsystem is NsdkOcclusionSubsystem nsdkSubsystem)
            {
                // Try to acquire the full image
                if (nsdkSubsystem.TryAcquireEnvironmentDepthCpuImage(default, out var image, out _))
                {
                    ...
                    // Get the matrices
                    intrinsics = nsdkSubsystem.LatestIntrinsicsMatrix ?? Matrix4x4.identity;
                    extrinsics = nsdkSubsystem.LatestExtrinsicsMatrix ?? Matrix4x4.identity;
                ...
                // Using NSDK, but failed to acquire the image
                texture = null;
                intrinsics = default;
                extrinsics = default;
                return false;
            ...
            if (_occlusionSubsystem is NsdkPlaybackOcclusionSubsystem nsdkPlaybackSubsystem)
            {
                intrinsics = nsdkPlaybackSubsystem.LatestIntrinsicsMatrix ?? Matrix4x4.identity;
                extrinsics = nsdkPlaybackSubsystem.LatestExtrinsicsMatrix ?? Matrix4x4.identity;
            }
            else
            {
        ...
        private bool ValidateOcclusionSubsystem()
        {
            ...
            if (_occlusionSubsystem == null)
            {
                Log.Warning
                (
                    "Destroying NsdkOcclusionEffect component because " +
            ...
            if (_occlusionSubsystem is NsdkOcclusionSubsystem nsdkSubsystem)
            {
                ...
                nsdkSubsystem.TargetFrameRate = _targetFrameRate;
            }
