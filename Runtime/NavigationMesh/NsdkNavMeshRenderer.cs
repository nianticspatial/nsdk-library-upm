...
{
    /// <summary>
    /// <c>NsdkNavMeshRenderer</c> is a helper <see cref="MonoBehaviour"/> which will draw the <see cref="NsdkNavMesh"/> tiles
    /// If you want to draw the <see cref="NsdkNavMesh"/> in a custom way you can create a similar renderer
    /// e.g. stylize the board as water/snow/sand etc.
    /// </summary>
    [PublicAPI("apiref/Niantic/Lightship/AR/NavigationMesh/NsdkNavMeshRenderer/")]
    public class NsdkNavMeshRenderer : MonoBehaviour
    {
        [FormerlySerializedAs("_lightshipNavMeshManager")]
        [FormerlySerializedAs("_navMeshManager")]
        [FormerlySerializedAs("_gameboardManager")]
        [Tooltip("The LightshipNavMeshManager that owns the NsdkNavMesh to render.")]
        [SerializeField]
        ...
        {
            //this will build a mesh for the NsdkNavMesh nav tiles
            ...
            float offset = 0.002f;
            var halfSize = _nsdkNavMeshManager.NsdkNavMesh.Settings.TileSize / 2.0f;
            foreach (var surface in _nsdkNavMeshManager.NsdkNavMesh.Surfaces)
            {
                foreach (var center in surface.Elements.Select
                         (node => Utils.TileToPosition(node.Coordinates, surface.Elevation,
                             _nsdkNavMeshManager.NsdkNavMesh.Settings.TileSize)))
                {
