...
using System.Linq;
using NianticSpatial.NSDK.AR.NavigationMesh.Internal;
...
{
  /// <summary>
  /// This class manages the data structures associated with a navigation mesh (<c>"NsdkNavMesh"</c>).
  /// It dynamically builds a 2d grid on the meshes detected in the environment for running navigation algorithms on.
  /// You are able to retrieve a number of properties of the <c>"NsdkNavMesh"</c> from it.
  /// There are also a number of methods to help you place and move <see cref="NsdkNavMeshAgent"/>s on the board.
  /// </summary>
  [PublicAPI]
  public class NsdkNavMesh
  {
    ...
    /// <param name="visualise">Visualize the <c>NsdkNavMesh</c> in the scene.</param>
    /// <returns>Returns the newly created <c>NsdkNavMesh</c>.</returns>
    ...
    /// <param name="delta"> The threshold distance from the NsdkNavMesh to check against.</param>
    /// <returns> <c>true</c> if the position provided is within delta meters of the NsdkNavMesh grid plane.</returns>
