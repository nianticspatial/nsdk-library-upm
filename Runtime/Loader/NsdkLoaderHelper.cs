...
{
    public class NsdkLoaderHelper
    {
        private readonly PlaybackLoaderHelper _playbackLoaderHelper;
        private readonly NativeLoaderHelper _nativeLoaderHelper;

        private INsdkInternalLoaderSupport _loader;
        private readonly List<INsdkExternalLoader> _externalLoaders;

        // Constructs NativeLoaderHelper and PlaybackLoaderHelper components based on the given settings
        internal NsdkLoaderHelper(List<INsdkExternalLoader> externalLoaders = null)
        {
            _externalLoaders = externalLoaders ?? new List<INsdkExternalLoader>();
            _nativeLoaderHelper = new NativeLoaderHelper();
            if (NsdkSettingsHelper.ActiveSettings.UsePlayback)
            {
        ...
        internal bool Initialize(INsdkInternalLoaderSupport loader)
        {
            ...
            // NSDK loader initializes as non-playback relies on checking the existence of arf's meshing subsystem
            ...
            {
                foreach(INsdkExternalLoader externalLoader in _externalLoaders)
