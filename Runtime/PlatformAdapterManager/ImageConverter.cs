...
using NianticSpatial.NSDK.Spaces;
...
namespace NianticSpatial.NSDK.AR.PAM
{
    internal static class ImageConverter
    {
...
#if NIANTICSPATIAL_NSDK_SPACES_ENABLED
        ...
        ///
        /// Note: For our computer vision algorithms in NSDK we always expect landscape right as orientation.
        ...
        )
        {
            if (s_material == null)
            {
                var nsdkImageConversionShader = Shader.Find("Unlit/NsdkImageConversion");
                if (nsdkImageConversionShader == null)
                {
                    Log.Error("Could not locate Unlit/NsdkImageConversion shader");
                    return;
                }
                s_material = new Material(nsdkImageConversionShader);
            }
