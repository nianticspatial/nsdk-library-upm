...
{
    public struct NsdkCpuImagePlane
    {
        public NsdkCpuImagePlane(IntPtr dataPtr, uint dataSize, uint pixelStride, uint rowStride)
        {
            DataPtr = dataPtr;
            DataSize = dataSize;
    ...
    /// </summary>
    public struct NsdkCpuImage
    {
        public const int MaxPlanes = 3;

        public static bool TryGetFromXRCpuImage(XRCpuImage xrCpuImage, out NsdkCpuImage nsdkCpuImage)
        {
            if (!xrCpuImage.valid)
            {
                nsdkCpuImage = default;
                return false;
            }

            NsdkCpuImagePlane[] planes = new NsdkCpuImagePlane[MaxPlanes];
            ...
            // Convert Unity format to NSDK format
            var format = GetFormatCEnum(xrCpuImage);
            if (format == ImageFormatCEnum.Unknown)
            {
                Debug.LogError($"NsdkCpuImage: Unsupported XRCpuImage format: {xrCpuImage.format}");
                nsdkCpuImage = default;
                return false;
            }
            ...
            {
                planes[1] = planes[2];
                planes[2] = new NsdkCpuImagePlane(IntPtr.Zero, 0, 0, 0);
            }
        ...
        public static NsdkCpuImage Create()
        {
            return new NsdkCpuImage
            {
                Planes = new NsdkCpuImagePlane[MaxPlanes],
        ...
        public NsdkCpuImagePlane[] Planes;
