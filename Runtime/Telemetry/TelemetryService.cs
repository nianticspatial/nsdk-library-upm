...
namespace NianticSpatial.NSDK.AR.Telemetry
{
    internal class TelemetryService : IDisposable
    {
        ...
        private readonly string _nsdkApiKey;
        ...
        public TelemetryService(IntPtr nsdkUnityContext, ITelemetryPublisher telemetryPublisher, string nsdkApiKey)
        {
            _protoMessageParser = new MessageParser<ArdkNextTelemetryOmniProto>(() => new ArdkNextTelemetryOmniProto());
            _telemetryPublisher = telemetryPublisher;
            _nsdkApiKey = nsdkApiKey;

            // Do not initialise native layer for Windows
            if (nsdkUnityContext.IsValidHandle())
            {
                _telemetryServiceHandle =
                    Lightship_ARDK_Unity_Telemetry_GetTelemetryServiceHandle(nsdkUnityContext);
            }
        ...
        public static void PublishEvent(ArdkNextTelemetryOmniProto nsdkNextTelemetryOmniProto)
        {
            PublishEvent(nsdkNextTelemetryOmniProto, requestId: string.Empty);
        }

        public static void PublishEvent(ArdkNextTelemetryOmniProto nsdkNextTelemetryOmniProto, string requestId)
        {
            ...
            if (nsdkNextTelemetryOmniProto.TimestampMs == default)
            {
                nsdkNextTelemetryOmniProto.TimestampMs = GetCurrentUtcTimestamp();
            }

            nsdkNextTelemetryOmniProto.ArCommonMetadata = Metadata.GetArCommonMetadata(requestId);
            s_messagesToBeSent.Enqueue(nsdkNextTelemetryOmniProto);
        }
                    ...
                    unsafe
                    {
                        ...
                        for (int i = 0; i < eventCount; i++)
                        {
                            ...
                            if (TryParseEvent(bytesArray, out var omniProto))
                            {
                                ...
                                omniProto.DeveloperKey = _nsdkApiKey;
        ...
        private bool TryParseEvent(NativeArray<byte> serialisedPayload, out ArdkNextTelemetryOmniProto nsdkNextTelemetryOmniProto)
        {
            try
            {
                nsdkNextTelemetryOmniProto = _protoMessageParser.ParseFrom(serialisedPayload.ToArray());
                return true;
            }
            catch (Exception e)
            {
            ...
            nsdkNextTelemetryOmniProto = null;
            return false;
        }
        ...
        private void PublishQueuedEvents()
        {
            while (!s_messagesToBeSent.IsEmpty)
            {
                ...
                {
                    eventToPublish.DeveloperKey = _nsdkApiKey;
