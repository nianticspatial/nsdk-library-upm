...
{
    // This feature enumeration corresponds to NSDK_Feature in the native implementation
    internal enum Feature : byte
    {
        [Description("None")] Unspecified = 0,
        [Description("NSDK Depth")] Depth,
        [Description("NSDK Semantic Segmentation")] Semantics,
        [Description("NSDK Scanning SQC")] ScanningSQC,
        [Description("NSDK Object Detection")] ObjectDetection
    }
    ...
    internal sealed class NativeModelPreloader : IModelPreloader
    {
        ...
        {
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_Create")]
            public static extern IntPtr Create(IntPtr unityContext);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_Release")]
            public static extern void Release(IntPtr preloaderHandle);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_DownloadModel")]
            public static extern PreloaderStatusCode DownloadModel(IntPtr preloaderHandle, byte feature, byte mode);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_RegisterModel")]
            public static extern PreloaderStatusCode RegisterModel(IntPtr preloaderHandle, byte feature, byte mode, string filepath);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_CurrentProgress")]
            ...
            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_ExistsInCache")]
            public static extern bool ExistsInCache(IntPtr preloaderHandle, byte feature, byte mode);

            [DllImport(NsdkPlugin.Name, EntryPoint = "Lightship_ARDK_Unity_Preloader_ClearFromCache")]
