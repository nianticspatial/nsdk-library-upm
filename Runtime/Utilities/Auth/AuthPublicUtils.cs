...
namespace NianticSpatial.NSDK.AR.Utilities.Auth
{
    ...
    public static class AuthPublicUtils
    {
        ...
        {
            var accessTail = AuthGatewayUtils.GetTokenShortName(token);
            var expiresAt = AuthGatewayUtils.Instance.DecodeJwtTokenBody(token)?.exp ?? 0;
            var currentTimeSeconds = (int)DateTimeOffset.UtcNow.ToUnixTimeSeconds();
            string accessTimeLeft = AuthGatewayUtils.GetTimeLeftString(expiresAt, currentTimeSeconds);
            Debug.Log($"{context} token: {accessTail}, timeLeft: {accessTimeLeft}.");
        }
    }
}
